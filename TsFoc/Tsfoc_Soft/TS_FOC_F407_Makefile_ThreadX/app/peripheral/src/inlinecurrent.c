...
void DriftOffsets(Inlinecurrent_Typedef *Sensor)
{
    ...
    for(int i = 0; i < detect_rounds; i++)
    {
        Sensor->offset_ia += adc_buffer[Sensor->A]*_ADC_CONV;
        Sensor->offset_ib += adc_buffer[Sensor->B]*_ADC_CONV;
		Sensor->offset_ic += adc_buffer[Sensor->C]*_ADC_CONV; // 如果适用
    }
    Sensor->offset_ia /= detect_rounds;
    Sensor->offset_ib /= detect_rounds;
    Sensor->offset_ic /= detect_rounds;
}
// /// @brief 初始化电流传感器
// /// @param Sensor 
...
void CurrSense_Init(Inlinecurrent_Typedef *Sensor)
{
    ...
    const float _shunt_resistor = 0.01f; 
    const float amp_gain = 50.0f; 
    
    const float vlots_to_amps = 1.0f / _shunt_resistor / amp_gain;
...
void Current_Update(Inlinecurrent_Typedef *Sensor)
{
    ...
    float tran_vol_c = (float)adc_buffer[Sensor->C]*_ADC_CONV; // 如果适用
#ifdef DEBUG_CURRENT
    printf("%f,",tran_vol_a);
	printf("%f,",tran_vol_b);
	printf("%f\r\n",tran_vol_c);
